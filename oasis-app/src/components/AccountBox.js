...
const migrateUrl = 'https://This App.app/trade/account';
...
const WalletBalances = ({ hasActiveAccount, closeSidebarDrawer }) => {
    ...
    return ( <
        ...
        fontWeight = "bold"
        t = "smallCaps"
        width = "20%" > { lang.sidebar.asset } <
        /Text> <
        Text color = "steel"
        fontWeight = "bold"
        t = "smallCaps"
        width = "30%" > { lang.sidebar.balance } <
        /Text> <
        Text color = "steel"
        fontWeight = "bold"
        t = "smallCaps"
        width = "30%" > { lang.sidebar.usd } <
        /Text> <
        Box width = "20%" / >
        <
        /Flex>
        ...
        StripedRows > {
            tokenBalances.map(
                ...
                (!collapsed || idx < 4) && ( <
                    TokenBalance key = { `tokenbalance_${idx}` }
                    symbol = { symbol }
                    amount = { amount }
                    usdRatio = { usdRatio }
                    ...
                    button = {
                        ...
                        (symbol === 'DSR' ? ( <
                            ...
                            ActionButton onClick = {
                                () => trackBtnClick('Withdraw') } > { lang.actions.withdraw } <
                            /ActionButton> <
                            /Link>
                        ) : symbol === 'SAI' ? ( <
                            ActionButton onClick = {
                                () => trackBtnClick('Migrate') }
                            as = "a"
                            target = "_blank"
                            href = { migrateUrl } >
                            ...
                            ActionButton disabled = {!hasActiveAccount }
                            onClick = {
                                () => {
                                    trackBtnClick('Send', {
                                        collateral: token
...
function AccountBox({ currentAccount, closeSidebarDrawer }) {
    ...
    return ( <
        ...
        CardBody p = "s" >
        <
        WalletConnectDropdown show = { open }
        offset = { `-${getSpace('s') + 1}, 0` }
        openOnHover = { false }
        onClick = { toggleDropdown }
        close = { closeDropdown }
        trigger = { < ActiveAccount address = { address }
            type = { type }
            />} /
            >
            <
